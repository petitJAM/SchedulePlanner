...
CREATE TABLE `coursesinschedules` (
  ...
  KEY `SID_fk_idx` (`SID`),
  KEY `CID_fk_idx` (`CID`),
  CONSTRAINT `SID_fk` FOREIGN KEY (`SID`) REFERENCES `schedules` (`SID`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `CID_fk` FOREIGN KEY (`CID`) REFERENCES `courses` (`CID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1$$
